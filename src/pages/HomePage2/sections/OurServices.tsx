...
const services = [
  ...
  { name: 'Decks & Outdoor', desc: 'Custom decks, patios & outdoor living spaces built to last.', href: '/projects' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
          {services.map(s => (
            <Link key={s.name} to={s.href} style={{ background: '#fff', border: '1px solid #E0D8CC', borderRadius: '10px', padding: '28px 22px', borderTop: '3px solid #1A305E', textDecoration: 'none', display: 'block', transition: 'transform 0.2s' }}
              ...
              <div style={{ color: '#1A305E', fontSize: '16px', fontWeight: 700, marginBottom: '8px' }}>{s.name}</div>
              <div style={{ color: '#6A7A8A', fontSize: '13px', lineHeight: 1.6, marginBottom: '14px' }}>{s.desc}</div>
              <span style={{ color: '#B86B25', fontSize: '13px', fontWeight: 600 }}>Learn more →</span>
            </Link>
          ))}
        </div>
      </div>
