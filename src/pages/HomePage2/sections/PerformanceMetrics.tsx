    ...
    <div style={{ background: '#1A305E', display: 'flex', justifyContent: 'space-around', padding: 'clamp(24px, 3vw, 36px) clamp(20px, 4vw, 48px)', flexWrap: 'wrap', gap: '20px' }}>
      {stats.map(s => (
        <div key={s.lbl} style={{ textAlign: 'center', minWidth: '120px' }}>
          <div style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 800, color: '#fff' }}>{s.num}</div>
          <div style={{ fontSize: '13px', color: '#F5F2EA', letterSpacing: '1px', marginTop: '4px', textTransform: 'uppercase', opacity: 0.8 }}>{s.lbl}</div>
        </div>
      ))}
    </div>
  )
